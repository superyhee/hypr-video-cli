...
| effectType | Key Params | Description |
|-----------|------------|-------------|
| `filmGrain` | `intensity` (0-1), `size` (0.5-5), `speed` (0-5) | Film grain noise |
...
```json
{
  ...
  "properties": {
    "effectLayers": [
      { "effectType": "colorGrading", "effectParams": { "contrast": 0.2, "saturation": -0.1, "temperature": 0.1 }, "enabled": true },
      { "effectType": "vignette", "effectParams": { "intensity": 0.4, "radius": 0.8, "softness": 0.5 }, "enabled": true },
      { "effectType": "filmGrain", "effectParams": { "intensity": 0.3, "size": 1.5 }, "enabled": true }
    ]
  }
}
```
...
**Cinematic:** colorGrading (contrast +0.2, saturation -0.1) + vignette + filmGrain

**Retro/VHS:** vhs (distortion 0.5, noise 0.3) + filmGrain + colorShift

**Dream:** gaussianBlur (radius 3) + glow (intensity 0.4) + lightLeak

**Glitch:** rgbSplit + glitchBlock + shake

**Night:** vignette (intensity 0.6) + colorGrading (brightness -0.2) + filmGrain
...
```json
{
  ...
  "enabled": true,
  "keyframes": [
    { "id": "kf_1", "time": 0, "properties": { "radius": 0 }, "easing": "easeOut" },
    { "id": "kf_2", "time": 2000, "properties": { "radius": 10 }, "easing": "easeIn" },
    { "id": "kf_3", "time": 4000, "properties": { "radius": 0 }, "easing": "linear" }
  ]
}
```
